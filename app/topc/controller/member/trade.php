...
class topc_ctl_member_trade extends topc_ctl_member {
    ...
    public function tradeList()
    {
        ...
        foreach( $tradelist as $key=>$row)
        {
            ...
            //通过请求api生成im的html内容
            //第一次尝试接口返回html数据。这样就可以并发生成页面了
          //$url = app::get('topc')->res_full_url . '/im.png';
          //$innerHTML = "<img src='$url' height='18'>";
          //$requestParams = ['shop_id'=>$row['shop_id'], 'type'=>'tradeList', 'content'=>$innerHTML, 'user_id'=>$user_id, 'params'=>['note'=>'订单号:'.$row['tid'] ] ];
          //$imHTML = app::get('topc')->rpcCall('im.get.html.row', $requestParams);
          //$tradelist[$key]['imHTML'] = $imHTML;
