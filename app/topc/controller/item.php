...
class topc_ctl_item extends topc_controller {
    ...
    public function index()
    {
        ...
        //获取是否免邮的信息
        $dlytmplInfo = app::get('topc')->rpcCall('logistics.dlytmpl.get',$dlytmplParams);
        if($dlytmplInfo)
        {
            $pagedata['freeConf'] = $dlytmplInfo['is_free'];
        }
        //echo '<pre>';var_dump($pagedata['freeConf']);exit();
        //面包屑数据
        ...
        $pagedata['imurl'] = app::get('topc')->res_full_url . '/im.png';
         //echo '<pre>';print_r($pagedata['collect']);exit();
    ...
    {
        $userId = userAuth::id();
        $collect = unserialize($_COOKIE[$userId.'collect']);
    ...
    private function __breadcrumb($catId,$brandId)
    {
        $cat = app::get('topc')->rpcCall('category.cat.get.data',array('cat_id'=>intval($catId)));
        $brand = app::get('topc')->rpcCall('category.brand.get.info',array('brand_id'=>$brandId,'fields'=>'brand_id,brand_name'));
        $breadcrumb = [
            ['url'=>url::action('topc_ctl_topics@index',array('cat_id'=>$cat['lv1']['cat_id'])),'title'=>$cat['lv1']['cat_name']],
            ['url'=>'','title'=>$cat['lv2']['cat_name']],
            ['url'=>url::action('topc_ctl_list@index',array('cat_id'=>$cat['lv3']['cat_id'])),'title'=>$cat['lv3']['cat_name']],
            ['url'=>url::action('topc_ctl_list@index',array('brand_id'=>$brand['brand_id'])),'title'=>$brand['brand_name']],
        ];
        return $breadcrumb;
    }
}
