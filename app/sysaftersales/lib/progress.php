...
class sysaftersales_progress {
     ...
     */
    public function check($filter, $result, $explanation=null, $refundsData=null,$shopId,$refundsReason=null)
    {
        $info = $this->objMdlAftersales->getRow('aftersales_bn,aftersales_type,progress,tid,oid,status,shop_id,user_id,reason',$filter);
        ...
        else
        {
            ...
            if( $info['aftersales_type'] == 'ONLY_REFUND'  || ($info['progress'] == '2' && $info['aftersales_type'] == 'REFUND_GOODS'))
            {
                ...
                app::get('sysaftersales')->rpcCall('aftersales.refundapply.create', $refundsData);
