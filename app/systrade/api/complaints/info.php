...
class systrade_api_complaints_info {
    ...
    public function getparams()
    {
        //接口传入的参数
        $return['params'] = array(
            'oid' => ['type'=>'int', 'valid'=>'', 'default'=>'', 'example'=>'1507021608160001','description'=>'子订单ID'],
            'complaints_id' => ['type'=>'int', 'valid'=>'', 'default'=>'', 'example'=>'15','description'=>'投诉自增id'],
    ...
    public function get($params)
    {
        ...
        $filter = array();
        if($params['complaints_id'])
        {
            $filter['complaints_id'] = $params['complaints_id'];
        }
        if($params['oid'])
        {
            $filter['oid'] = $params['oid'];
        }
        return kernel::single('systrade_data_complaints')->getDetailInfo($filter, $userId ,$params['fields']['rows'], $params['fields']['extends']['orders']);
    }
}
