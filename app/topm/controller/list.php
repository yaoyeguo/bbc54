   ...
   private function __getItems($postdata)
    {
        ...
        $params['search_keywords'] = htmlspecialchars($params['search_keywords']);
        ...
        {
            $itemIds = array_keys($itemsList['list']);
            $cookieItem = $_COOKIE[$userId.'collect'];
            $cookieItemIds = unserialize($cookieItem)['item'];
            $intersectIds = array_intersect($itemIds,$cookieItemIds);
            foreach ($intersectIds as  $itemId)
            {
                $itemsList['list'][$itemId]['itemCollect'] = 1;
            }
        }
