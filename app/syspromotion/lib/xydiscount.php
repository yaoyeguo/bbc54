...
class syspromotion_xydiscount extends syspromotion_abstract_promotions {
    ...
    private function __preareData($data) {
        ...
        //$objMdlXydiscountItem = app::get('syspromotion')->model('xydiscount_item');
        //$itemList = $objMdlXydiscountItem->getList('xydiscount_id, title', array('item_id'=>$aResult['rel_item_ids'], 'end_time|than'=>time(), 'status'=>1 ) );
    ...
    /**
     * 取消X件Y折促销
     * @param  array $params X件Y折促销更新过滤条件
     * @return boolen
     */
    public function xydiscountCancel($params)
    {
        ...
        {
            // 修改X件Y折促销状态
            if( !$objMdlXydiscount->update( array('xydiscount_status'=>'cancel'), array('xydiscount_id'=>$xydiscountId, 'shop_id'=>$params['shop_id']) ) )
            {
                throw new \LogicException(app::get('syspromotion')->_('取消X件Y折促销失败'));
            }

            // 修改X件Y折促销关联商品的状态
            //$objMdlXydiscountItem = app::get('syspromotion')->model('xydiscount_item');
            //if( !$objMdlXydiscountItem->update( array('status'=>1), array('xydiscount_id'=>$xydiscountId, 'shop_id'=>$params['shop_id']) ) )
            //{
            //    throw new \LogicException(app::get('syspromotion')->_('取消X件Y折促销失败'));
            //}
            ...
            // 修改促销关联表的促销状态
            if( !$this->savePromotions( array('rel_promotion_id'=>$xydiscountId,'promotion_type'=>'xydiscount','check_status'=>'cancel') ) )
            {
                throw new \LogicException(app::get('syspromotion')->_('修改X件Y折促销状态'));
            }
            $db->commit();
