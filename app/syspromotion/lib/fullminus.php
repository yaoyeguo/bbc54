...
class syspromotion_fullminus extends syspromotion_abstract_promotions {
    ...
    private function __preareData($data) {
        ...
        //$objMdlFullminusItem = app::get('syspromotion')->model('fullminus_item');
        //$itemList = $objMdlFullminusItem->getList('fullminus_id, title', array('item_id'=>$aResult['rel_item_ids'], 'end_time|than'=>time(),'status'=>0 ) );
    ...
    public function fullminusCancel($params)
    {
        ...
        {
            // 修改满减促销状态
            if( !$objMdlFullminus->update( array('fullminus_status'=>'cancel'), array('fullminus_id'=>$fullminusId, 'shop_id'=>$params['shop_id']) ) )
            {
                throw new \LogicException(app::get('syspromotion')->_('取消满减促销失败'));
            }

            //修改满减促销商品的状态
            //$objMdlFullminusItem = app::get('syspromotion')->model('fullminus_item');
            //if(!$objMdlFullminusItem->update(array('status'=>1),array('fullminus_id'=>$fullminusId, 'shop_id'=>$params['shop_id'])) )
            //{
            //    throw new \LogicException(app::get('syspromotion')->_('取消满减促销失败'));
            //}
            ...
            // 修改促销关联表的促销状态
            if( !$this->savePromotions( array('rel_promotion_id'=>$fullminusId,'promotion_type'=>'fullminus','check_status'=>'cancel') ) )
            {
                throw new \LogicException(app::get('syspromotion')->_('修改满减促销状态'));
            }
            $db->commit();
