...
{
    private $baseInstalled = false;

    public function __construct()
    {
        if (array_get(db::getConfig(), 'host')!=='%CONNECTIONS.DEFAULT.HOST%') {
            $this->setBaseInstalled();
        }
    }

    public function isBaseInstalled()
    {
        return $this->baseInstalled;
    }

    public function setBaseInstalled()
    {
    ...
    private function check_depends_install($app_id, &$queue){
        ...
        if(!$this->isBaseInstalled() || app::get($app_id)->status() == 'uninstalled'){
            $queue[$app_id] = app::get($app_id)->define();
        }
    }
