...
class base_cache_store_secacheEngine 
{
    ...
    public function __construct($workat, $file, $size = '1g')
    {
    ...
    function clean(){
        ...
        return $files->delete($this->_file);
    }

    function fetch($key,&$return, $isLock = true)
    {
        if($isLock)
        {
            if ($this->lock(false))
    ...
    function storePayload($key, $value)
    {
    ...
    function increment($key, $value =1, $initial = 0, $minutes)
    {
        if($this->lock(true))
        {
            $this->fetch($key, $origData, false);
            //var_dump($kkk);exit;
            $data = unserialize($origData);
            //todo: 增加 time 逻辑
            $data['content'] = intval($data['content']) + $value;
            //var_dump($data);
            $this->storePayload($key, serialize($data));
            //save data
            $this->unlock();
            return $data['content'];
            //return true;
