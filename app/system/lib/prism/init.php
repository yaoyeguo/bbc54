...
class system_prism_init
{
    ...
    private function __initApi()
    {
        ...
        $area = config::get('prism.appPushArea');
    ...
    private function __updateApi()
    {
        ...
        {
            $appPrismId = system_prism_init_util::getAppId($apiKey);
            if($appPrismId != null)
            {
    ...
    /**
     * 判断app是否会被push到prism上面
     *
     * @return ecapp array ecos框架的app(非实例化的)。保存在base_apps表中
     */
    private function __getEcapps()
    {
        $prismInitStepModel = app::get('system')->model('prism_initstep');
        ...
        foreach($ecapps as $ecapp)
        {
            ...
            //
            //if(in_array( $ecapp['type'] ? $ecapp['type'] : app::get($ecapp['app_id'])->define('type'), $area))
            //这里逻辑修改为push的app
            if(in_array( $ecapp['app_id'], $area))
            {
                $fmtEcapps[] = $ecapp;
            }
        }
