...
class system_ctl_admin_messenger extends desktop_controller{
    ...
    public function setSmsConf($action,$msg)
    {
        $count = app::get('system')->getConf('messenger.count.'.$action.'.'.$msg);
        $active = app::get('system')->getConf('messenger.ttl.'.$action.'.'.$msg);
        $pagedata = array();
        $pagedata['tpl'] = $action;
        $pagedata['type'] = $msg;
        $pagedata['count'] = $count;
        $pagedata['active'] = $active;
        return $this->page('system/admin/messenger/setsmsconf.html', $pagedata);
    }
    
    public function saveSmsConf()
    {
        $postData = input::get();
        $objMessenger = kernel::single('system_data_messenger');
        try {
            $objMessenger->setSmsConf($postData);
            $this->adminlog("短信验证码设置", 1);
        } catch (Exception $e) {
            $this->adminlog("短信验证码设置", 0);
            $msg = $e->getMessage();
            return $this->splash('error',null,$msg);
        }
        
        return $this->splash('success',null,"短信验证码配置保存成功");
    }
