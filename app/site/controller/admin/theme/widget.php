...
class site_ctl_admin_theme_widget extends site_admin_controller
{
    ...
    public function has_lock()
    {
        ...
        {
            $filekey = $theme.$urlType.md5($file);
            $themeDataCache = cache::store('misc')->get($filekey);
            if (!$themeDataCache)
            {
                cache::store('misc')->put($filekey, true, 5);//5分钟内锁定
                setcookie($filekey, true, time()+300, '/');
            }
            elseif(!$_COOKIE[$filekey])
            {
                exit('模板可能正在被他人编辑，请稍后重试！');
            }
        }
    }
    ...
    public function editor()
    {
        ...
        $this->has_lock(); //检查当前编辑模板文件是否正在被编辑，防止同时编辑模板带来的困扰
    ...
    public function preview()
    {
        ...
        $result = $theme->layout($layout)->render();
        cache::enable();
        return $result;
...
//        header("Cache-Control:no-store, no-cache, must-revalidate"); //强制刷新IE缓存
    ...
    public function save_all()
    {
        ...
        //清除对应的模板缓存
        kernel::single('site_theme_tmpl')->touch_theme_tmpl();
